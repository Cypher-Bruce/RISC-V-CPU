...
- **`/Assembly`**: Assembly code for the project, the tools to convert the assembly code to `.coe` file and `.txt` file and the tools to load the program to the FPGA.
...
- **`Pipeline_CPU.xpr`**: The Vivado project file.
- **`Top.bit`**: The pre-built bitstream for the project.
